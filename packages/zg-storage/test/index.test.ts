...
import {SpecRootMismatchError, ZgStore, zgMerkleRoot} from "../src/index";
...
describe("ZgStore", () => {
  it("returns the document when its bytes hash to the root asked for", async () => {
    ...
    const doc = (await new ZgStore(INDEXER, fetchImpl).get(LIVE_ROOT)) as Record<string, unknown>;
    expect(doc.question).toContain("ETH/USD");
    expect(doc.rules).toContain("Resolves YES");
  ...
  it("calls the global fetch with the global as its receiver, not the store", async () => {
    ...
    try {
      await new ZgStore(INDEXER).get(LIVE_ROOT);
    } finally {
      vi.unstubAllGlobals();
    }
